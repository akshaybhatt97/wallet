...
export async function postTransaction(req, res) {
  ...
  try {
    ...
    await db
      ...
      .insertOne({ ...transaction, date: dayjs().format("DD/MM"), userId: session.userId });
...
export async function deleteTransaction(req, res) {
  ...
  try {
    ...
    if (session.userId.toString() !== transaction.userId.toString())
      return res.status(401).send("This transaction is not yours");
...
export async function updateTransaction(req, res) {
  ...
  try {
    ...
    if (!/\.[0-9]{2}$/.test(editTransaction.value))
      editTransaction.value = parseFloat(editTransaction.value).toFixed(2);
